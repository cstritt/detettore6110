...
def core_dist_stats(dist):
    ...
    influence of coverage and insert size outliers in the bam file.
    """
...
def length_stats(bamfile):
    ...
    rl_stdev = int(statistics.stdev(readlengths)) if len(readlengths) > 1 else 0
    
    read_d = {
        'readlength_max' : max(readlengths),
        'readlength_mean' : int(statistics.mean(readlengths)),
        'readlength_stdev' : rl_stdev
        } 
